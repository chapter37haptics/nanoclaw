...
export async function executeAction(action: ActionFile): Promise<ActionResult> {
  ...
  try {
    switch (action.type) {
      ...
      case 'mcp_add': {
        const { key, config } = action.payload as {
          key: string;
        ...
        return {
          status: 'success',
          summary: `MCP server "${key}" added to settings.json.`,
        };
      }
      ...
      case 'skill_write': {
        ...
        return {
          status: 'success',
          summary: `Skill "${skill_name}" written to ~/.claude/skills/.`,
        };
      }
      ...
      case 'claude_md_append': {
        ...
        return {
          status: 'success',
          summary: 'Appended to ~/.claude/CLAUDE.md.',
        };
      }
      ...
      default:
        return {
          status: 'error',
          error: `Unknown action type: ${(action as ActionFile).type}`,
        };
    }
  } catch (err) {
